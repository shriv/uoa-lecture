...
The notebook (`demo.ipynb`) depends on datasets filtered to `Auckland Central` suburb which are committed to the repo. Only `uv` is needed for running the notebook. All the modules, demo notebook and data are in one folder to make paths easier. Run `uv sync` to install the necessary dependencies. 
